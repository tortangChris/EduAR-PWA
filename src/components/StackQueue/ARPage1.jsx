...
const ARPage1 = () => {
  ...
  const [structurePos, setStructurePos] = useState([-2, 0, -10]);
  ...
  const positions = useMemo(() => stack.map((_, i) => [0, i * spacing, 0]), [stack, spacing]);
...
const ARInteractionManager = ({ 
    ...
    const onSessionStart = () => {
      ...
      const onSelectEnd = () => {
        ...
        if (isDraggingRef.current) {
          ...
          onSelectAction(touchedButton.current);
        }
  ...
  }, [gl, buttonRefs, structureRef, onSelectAction, onDragStart, onDragMove, onDragEnd]);
...
const DefinitionPanel = ({ position, onNextClick, stack }) => {
  ...
  return (
    <group>
      ...
      <Text
        position={[position[0], position[1] - 3.8, position[2]]}
        fontSize={0.45}
        color="#38bdf8"
        anchorX="center"
        anchorY="middle"
