...
const VisualPage2 = ({ stepDuration = 700 }) => {
  ...
  function createBoxes(arr, compare = [], sortedIdx = -1) {
    const n = arr.length;
    const mid = (n - 1) / 2;
    return arr.map((val, i) => ({
      ...
      value: val,
      x: (i - mid) * 2.0,
      color: compare.includes(i)
        ? "#f59e0b" // orange for comparing
        : i >= sortedIdx
        ? "#22c55e" // green for sorted
  ...
  const sortArray = async () => {
    ...
    // Bubble Sort step recording
    for (let i = 0; i < n - 1; i++) {
      for (let j = 0; j < n - i - 1; j++) {
        steps.push({ arr: arr.slice(), compare: [j, j + 1], sorted: n - i });
        if (arr[j] > arr[j + 1]) {
          [arr[j], arr[j + 1]] = [arr[j + 1], arr[j]];
        }
      }
    }
    steps.push({ arr: arr.slice(), compare: [], sorted: n });

    // Play animation
    for (let step = 0; step < steps.length; step++) {
      if (animRef.current.cancelled) break;
      const { arr, compare, sorted } = steps[step];
      setBoxes(createBoxes(arr, compare, sorted));
